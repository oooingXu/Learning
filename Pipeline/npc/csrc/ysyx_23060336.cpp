...
uint32_t ifu_count, pipeline_pc, pipeline_dnpc;
bool pipeline_valid;
...
static void performance_count(bool ebreak, uint32_t i_type_count, uint32_t s_type_count, uint32_t u_type_count, uint32_t b_type_count, uint32_t r_type_count, uint32_t j_type_count, uint32_t c_type_count, uint32_t w_type_count, uint32_t i_clk, uint32_t s_clk, uint32_t u_clk, uint32_t b_clk, uint32_t r_clk, uint32_t j_clk, uint32_t c_clk, uint32_t w_clk) {
    ...
    // 打印信息
		if(ebreak) {
			...
			printf("数据冒险时钟占比: %.2f%, 结构冒险时钟占比： %.2f%\n", raw_data_avg_cycles, raw_control_avg_cycles);
...
extern "C" void exu_counter(uint32_t exu_clk_h, uint32_t exu_clk_l, uint32_t raw_control_clk_h, uint32_t raw_control_clk_l) {
...
extern "C" void set_npc_state(int ebreak, uint32_t i_type_count, uint32_t s_type_count, uint32_t u_type_count, uint32_t b_type_count, uint32_t r_type_count, uint32_t j_type_count, uint32_t c_type_count, uint32_t w_type_count, uint32_t i_clk, uint32_t s_clk, uint32_t u_clk, uint32_t b_clk, uint32_t r_clk, uint32_t j_clk, uint32_t c_clk, uint32_t w_clk, uint32_t wbu_clk_h, uint32_t wbu_clk_l){
	wbu_clk = wbu_clk_h << 32 | wbu_clk_l;

	IFDEF(CONFIG_COUNTER, performance_count(ebreak, i_type_count, s_type_count, u_type_count, b_type_count, r_type_count, j_type_count, c_type_count, w_type_count, i_clk, s_clk, u_clk, b_clk, r_clk, j_clk, c_clk, w_clk));
